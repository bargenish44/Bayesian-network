...
public class Algorithms {
	...
	public String Variable_elimination(String s) throws Exception {
		...
		String wantedNode=(str2[0].split("=")[0]);
	...
	private boolean ancestor(BayesianNetwork.Var v, ArrayList<Pair> ev,String wantedNode) {
		if(v.getName().equals(wantedNode))return true;
		for(Pair p:ev) {
			if(v.getName().equals(p.getFirst()))return true;
		}
		for(BayesianNetwork.Var child:v.childrens) {
			return ancestor(child,ev,wantedNode);
		}
		deletion.remove(v);
		return false;
	}
